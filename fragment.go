...
func (hf *HopFragmenter) Fragmentate(c hopSequencer, frame []byte) []*HopPacket {
	...
	/*
	   ...
	   hp.setPayload(frame)
	   fmt.Println("seq: %d, plen: %d, dlen: %d", seq, hp.Plen, hp.Dlen)
	...
	*/
	...
	for i, restSize := 0, frameSize; i < MAX_FRAGS; i++ {
		...
		//fmt.Println("restSize: %d, fragSize: %d", restSize, fragSize)
	...
	//fmt.Println("packSize: %d, fragSize: %v", frameSize, prefixes)
	last := len(packets) - 1
...
func (hf *HopFragmenter) reAssemble(packets []*HopPacket) []*HopPacket {
	...
	defer func() {
		...
		if err := recover(); err != nil {
			fmt.Println("Error reassemble packet fragments: %s", err)
		}
	}()

	for _, p := range packets {
		// fmt.Println("frag: %v", p.hopPacketHeader)
		if p.Dlen == p.Plen {
			// fmt.Println("rpacket: %v", p.hopPacketHeader)
		...
		if r, found := hf.cache.cache[p.Seq]; found {
			rp := r.p
			// fmt.Println("plen: %d, recved: %d", rp.Plen, rp.Dlen)
			rp.Dlen += p.Dlen
			s := p.FragPrefix
			e := s + p.Dlen
			...
			if rp.Dlen == rp.Plen {
				rp.Flag ^= HOP_FLG_MFR
				// fmt.Println("rpacket: %v", rp.hopPacketHeader)
